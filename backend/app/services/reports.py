...
REPORT_TYPE_TEXT = {"product": "商品举报", "user": "用户举报"}
REPORT_REASON_TEXT = {
    "fake_info": "虚假商品/描述不符",
    ...
    "fraud": "欺诈风险",
    "illegal_product": "发布违规商品",
    "malicious_trade": "恶意交易/恶意退款",
    "fake_identity": "冒充他人/虚假身份",
    "other": "其他",
}
...
class ReportService:
    ...
    def create_report(self, reporter_id, payload):
        target_type = (payload.get("target_type") or ("user" if payload.get("target_user_id") else "product")).strip()
        if target_type == "user":
            return self._create_user_report(reporter_id, payload)
        if target_type == "product":
            return self._create_product_report(reporter_id, payload)
        raise ValidationError("参数校验失败", [{"field": "target_type", "message": "举报类型不合法"}])

    def _create_product_report(self, reporter_id, payload):
        product_id = to_object_id(payload.get("product_id") or payload.get("target_id"), "product_id")
        ...
        if self.db.reports.find_one({"target_type": "product", "product_id": product_id, "reporter_id": ObjectId(str(reporter_id)), "status": "pending"}):
            raise ConflictError("你已提交过该商品的待处理举报")

        reason_type, description, evidence_images = self._validate_common_report(reporter_id, payload)
        ...
        doc = {
            **self._base_doc(reporter_id, reporter, reporter_profile, reason_type, description, evidence_images),
            "target_type": "product",
        ...
        self._notify_admin_pending(result.inserted_id, "商品举报")
        return self._present(self.db.reports.find_one({"_id": result.inserted_id}))

    def _create_user_report(self, reporter_id, payload):
        target_user_id = to_object_id(payload.get("target_user_id") or payload.get("target_id"), "target_user_id")
        if str(target_user_id) == str(reporter_id):
            raise ForbiddenError("不能举报自己")
        target_user = self.users.find_by_id(target_user_id)
        if not target_user or target_user.get("status") in {"disabled", "frozen", "banned"}:
            raise NotFoundError("被举报用户不存在")
        if self.db.reports.find_one({"target_type": "user", "target_user_id": target_user_id, "reporter_id": ObjectId(str(reporter_id)), "status": "pending"}):
            raise ConflictError("你已提交过该用户的待处理举报")

        reason_type, description, evidence_images = self._validate_common_report(reporter_id, payload)
        reporter = self.users.find_by_id(reporter_id) or {}
        reporter_profile = self.users.find_profile(reporter_id) or {}
        target_profile = self.users.find_profile(target_user_id) or {}
        doc = {
            **self._base_doc(reporter_id, reporter, reporter_profile, reason_type, description, evidence_images),
            "target_type": "user",
            "target_user_id": target_user_id,
            "target_user_nickname_snapshot": target_profile.get("nickname") or target_user.get("nickname") or "校园用户",
            "target_user_avatar_snapshot": normalize_image_url(target_profile.get("avatar") or target_profile.get("avatar_url") or target_user.get("avatar_url") or ""),
            "target_user_credit_snapshot": self.credit.get_score(target_user_id),
            "product_action": "none",
        }
        result = self.db.reports.insert_one(doc)
        self._notify_admin_pending(result.inserted_id, "用户举报")
        return self._present(self.db.reports.find_one({"_id": result.inserted_id}))

    def _validate_common_report(self, reporter_id, payload):
        ...
        if description:
            ContentModerationService(self.db).validate_fields(reporter_id, {"report": description})
        ...
        return reason_type, description, evidence_images

    def _base_doc(self, reporter_id, reporter, reporter_profile, reason_type, description, evidence_images):
        return {
            ...
            "evidence_images": normalize_image_list(evidence_images),
            "status": "pending",
            "admin_id": None,
            "admin_note": "",
            "credit_deduct": 0,
    ...
    def list_admin_reports(self, args):
        ...
        if status:
            ...
            query["status"] = status
        target_type = (args.get("target_type") or args.get("type") or "").strip()
        if target_type in {"product", "user"}:
            query["target_type"] = target_type
        data = self._list(query, args)
        data["pending_count"] = self.db.reports.count_documents({"status": "pending"})
        data["pending_product_count"] = self.db.reports.count_documents({"status": "pending", "target_type": {"$in": ["product", None]}})
        data["pending_user_count"] = self.db.reports.count_documents({"status": "pending", "target_type": "user"})
        return data
    ...
    def handle_report(self, report_id, admin_user, payload):
        ...
        product_action = "none"
        target_type = report.get("target_type") or "product"
        if result == "approved":
            if target_type == "product":
                ...
                target_user_id = report["seller_id"]
                related_product_id = report["product_id"]
                notify_action = "product_taken_down_by_report"
                notify_content = f"你的商品因举报成立已被下架，信用分扣除 {credit_deduct} 分"
            else:
                target_user_id = report["target_user_id"]
                related_product_id = None
                notify_action = "user_report_approved"
                notify_content = f"你因用户举报成立被扣除信用分 {credit_deduct} 分"
            if credit_deduct:
                self.credit.adjust(target_user_id, -credit_deduct, "deduct", report.get("reason_type") or "other", f"举报成立：{report.get('reason_text')}", operator_type="admin", operator_id=admin_user["_id"], related_report_id=report["_id"], related_product_id=related_product_id)
            self._notify_user(target_user_id, notify_action, notify_content, report, admin_user["_id"])
            ...
            if credit_deduct:
                self.credit.adjust(report["reporter_id"], -credit_deduct, "deduct", "malicious_report", "恶意举报扣分", operator_type="admin", operator_id=admin_user["_id"], related_report_id=report["_id"], related_product_id=report.get("product_id"))
            self._notify_user(report["reporter_id"], "report_malicious", f"你的举报被判定为恶意举报，信用分扣除 {credit_deduct} 分", report, admin_user["_id"])
            if target_type == "user":
                self._notify_user(report["target_user_id"], "user_report_rejected", "针对你的用户举报已处理，平台未认定违规", report, admin_user["_id"])
    ...
    def _present(self, report, detail=False):
        ...
        data["target_type_text"] = REPORT_TYPE_TEXT.get(target_type, "商品举报")
        ...
        if target_type == "user":
            data["target_user"] = {
                "id": str(report.get("target_user_id")),
                "nickname": report.get("target_user_nickname_snapshot", ""),
                "avatar": report.get("target_user_avatar_snapshot", ""),
                "credit_score": self.credit.get_score(report["target_user_id"]) if detail else report.get("target_user_credit_snapshot", 100),
            }
        else:
            ...
            if detail and report.get("seller_id"):
    ...
    def _notify_admin_pending(self, report_id, type_text):
        for admin in self.db.users.find({"roles": "admin", "status": "active"}):
            self.db.messages.insert_one({"conversation_id": f"system_{admin['_id']}", "type": "system", "sender_id": admin["_id"], "receiver_id": admin["_id"], "message_type": "system", "system_action": "admin_report_pending", "content": f"有新的{type_text}待处理", "report_id": report_id, "read_at": None, "created_at": utc_now()})

    def _notify_user(self, receiver_id, action, content, report, sender_id):
        self.db.messages.insert_one({"conversation_id": f"system_{receiver_id}", "type": "system", "sender_id": sender_id, "receiver_id": receiver_id, "message_type": "system", "system_action": action, "content": content, "report_id": report["_id"], "product_id": report.get("product_id"), "target_user_id": report.get("target_user_id"), "read_at": None, "created_at": utc_now()})
